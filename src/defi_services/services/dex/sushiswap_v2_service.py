...
from defi_services.services.dex.dex_info.sushiswap_info import (SUSHISWAP_V2_ETH_INFO, SUSHISWAP_V2_FANTOM_INFO,
                                                                SUSHISWAP_V2_POLYGON_INFO, SUSHISWAP_V2_OPTIMISM_INFO,
                                                                SUSHISWAP_V2_AVALANCHE_INFO, SUSHISWAP_V2_ARBITRUM_INFO,
                                                                SUSHISWAP_V2_BSC_INFO)
...
class SushiSwapV2Info:
    mapping = {
        ...
        Chain.optimism: SUSHISWAP_V2_OPTIMISM_INFO,
...
class SushiSwapV2Services(DexProtocolServices):
    ...
    def get_all_supported_lp_token(self, limit: int = 1):
        ...
        if web3.isAddress(self.master_chef_addr):
            self.master_chef_addr = web3.toChecksumAddress(self.master_chef_addr)
        master_chef_contract = web3.eth.contract(abi=self.master_chef_abi, address=self.master_chef_addr)
        ...
        for pid in range(0, int(pool_length / limit)):
            ...
            rpc_calls[query_id] = self.state_service.get_function_info(
                address=self.master_chef_addr, abi=self.master_chef_abi, fn_name="lpToken", fn_paras=[pid]
            )
    ...
    def get_lp_token_function_info(self, supplied_data, block_number: int = "latest"):
        ...
        for lp_token, pid in lp_token_info.items():
            ...
            rpc_calls[query_id] = self.state_service.get_function_info(
                address=lp_token, abi=LP_TOKEN_ABI, fn_name="balanceOf", fn_paras=[self.master_chef_addr],
                block_number=block_number)

            query_id = f'poolInfo_{lp_token}_{block_number}_{self.chain_id}'.lower()
            rpc_calls[query_id] = self.state_service.get_function_info(
                address=self.master_chef_addr, abi=self.master_chef_abi, fn_name="poolInfo", fn_paras=[int(pid)],
                block_number=block_number)
        return rpc_calls
    ...
    def decode_lp_token_info(self, supplied_data, response_data, block_number: int = "latest"):
        ...
        for lp_token, pid in lp_token_info.items():
            ...
            total_supply = response_data.get(f'totalsupply_{lp_token}_{block_number}_{self.chain_id}'.lower(),
                                             0) / 10 ** decimals
            master_chef_balance = response_data.get(
            ...
            result[lp_token] = {
                ...
                "decimals": decimals,
                "name": name,
                "stakeBalance": master_chef_balance,
    ...
    def get_user_info_function(
        ...
        for lp_token, value in lp_token_info.items():
            ...
            if stake:
                ...
                rpc_calls[query_id] = self.state_service.get_function_info(
                    address=self.master_chef_addr, abi=self.master_chef_abi, fn_name="userInfo",
    ...
    def decode_user_info_function(
            self, user: str, supplied_data, user_data: dict, stake: bool = True,
    ...
    def get_rewards_balance_function_info(self, user, supplied_data, block_number: int = "latest"):
        ...
        for lp_token, value in lp_token_info.items():
            ...
            rpc_calls[query_id] = self.state_service.get_function_info(
                address=self.master_chef_addr, abi=self.master_chef_abi, fn_name="pendingSushi",
