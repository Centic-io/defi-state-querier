...
class CompoundV3StateService(CompoundStateService):
    ...
    def get_apy_lending_pool_function_info(
        ...
        for token_address, reserve_info in reserves_info.items():
            ...
            for fn_name in functions:
                multicall_calls.append(self.get_comet_function_info(
                    comet=comet_address, fn_name=fn_name, block_number=block_number))

            for collateral_address in reserve_info['assets']:
                multicall_calls.append(W3Multicall.Call(
                    address=collateral_address, abi=ERC20_ABI, fn_name="decimals", block_number=block_number))
                multicall_calls.append(self.get_comet_function_info(
                    comet=comet_address, fn_name="totalsCollateral",
                    fn_paras=self._w3.to_checksum_address(collateral_address), block_number=block_number))
    ...
    def get_rewards_balance_function_info(
        ...
        for token, value in self.pool_info.get("reservesList").items():
            comet = self._w3.to_checksum_address(value.get("comet"))
            multicall_calls.append(W3Multicall.Call(
                address=reward_address, abi=self.reward_abi, fn_name="getRewardOwed",
                fn_paras=[comet, self._w3.to_checksum_address(wallet)], block_number=block_number))
    ...
    def calculate_rewards_balance(
        ...
        for token, value in self.pool_info.get("reservesList").items():
            ...
            get_reward_id = f"getRewardOwed_{reward_address}_{[comet, wallet]}_{block_number}".lower()
    ...
    def get_wallet_deposit_borrow_balance_function_info(
        ...
        for token, value in reserves_info.items():
            ...
            multicall_calls.append(W3Multicall.Call(
                address=comet, abi=self.comet_abi, fn_name="borrowBalanceOf",
                fn_paras=wallet, block_number=block_number))

            for asset in assets:
                multicall_calls.append(W3Multicall.Call(
                    address=comet_ext, abi=self.comet_ext, fn_name="collateralBalanceOf",
                    fn_paras=[wallet, self._w3.to_checksum_address(asset)], block_number=block_number))
                multicall_calls.append(W3Multicall.Call(
    ...
    def calculate_wallet_deposit_borrow_balance(
        ...
        for token, value in reserves_info.items():
            ...
            borrow_key = f"borrowBalanceOf_{comet}_{wallet}_{block_number}".lower()
            decimals = decoded_data[get_decimals_id]
            borrow_amount = decoded_data[borrow_key] / 10 ** decimals
            for asset, asset_info in assets.items():
                balance_key = f"collateralBalanceOf_{comet_ext}_{[wallet, asset]}_{block_number}".lower()
    ...
    def get_comet_function_info(self, comet: str, fn_name: str, fn_paras: list = None, block_number: int = "latest"):
        return W3Multicall.Call(
            address=comet, abi=self.comet_abi, fn_name=fn_name,
