...
class CompoundStateService(ProtocolServices):
    ...
    def get_service_info(self):
        info = {
            "compound": {
                ...
                "type": "lending",
                "protocol_info": self.compound_info
            }
        }
        return info
    ...
    def get_dapp_asset_info(
        ...
        comptroller_contract = _w3.eth.contract(
            address=_w3.toChecksumAddress(self.compound_info.get("comptrollerAddress")), abi=self.comptroller_abi)
        ctokens = []
        ...
        lens_contract = _w3.eth.contract(
            address=Web3.toChecksumAddress(self.compound_info.get("lensAddress")), abi=self.lens_abi
        )
    ...
    def get_token_list(self):
        begin = time.time()
        tokens = [self.compound_info.get('rewardToken'), self.compound_info.get("poolToken")]
        for token in self.compound_info.get("reservesList"):
    ...
    def get_data(
        ...
        begin = time.time()
        reserves_info = kwargs.get("reserves_info", self.compound_info.get("reservesList"))
    ...
    def get_function_info(
        ...
        if not reserves_info:
            reserves_info = self.compound_info['reservesList']
        rpc_calls = {}
        if Query.deposit_borrow in query_types and wallet:
    ...
    def calculate_apy_lending_pool_function_call(
        ...
        pool_token_price = token_prices.get(self.compound_info.get("poolToken"))
    ...
    def get_rewards_balance_function_info(
        ...
        token = self.compound_info.get("poolToken")
        fn_paras = [Web3.toChecksumAddress(token),
                    Web3.toChecksumAddress(self.compound_info.get("comptrollerAddress")),
    ...
    def calculate_rewards_balance(self, wallet_address: str, decoded_data: dict, block_number: int = "latest"):
        ...
        reward_token = self.compound_info.get("rewardToken")
        result = {
    ...
    def get_lens_function_info(self, fn_name: str, fn_paras: list, block_number: int = "latest"):
        return self.state_service.get_function_info(
            self.compound_info['lensAddress'], self.lens_abi, fn_name, fn_paras, block_number
        )

    def get_comptroller_function_info(self, fn_name: str, fn_paras: list, block_number: int = "latest"):
        return self.state_service.get_function_info(
            self.compound_info['comptrollerAddress'], self.comptroller_abi, fn_name, fn_paras, block_number
        )
    ...
    def get_ctoken_metadata_all(
        ...
        key = f"cTokenMetadataAll_{self.compound_info.get('lensAddress')}_{block_number}".lower()
        return {
    ...
    def ctoken_underlying_price_all(
        ...
        tokens = [Web3.toChecksumAddress(value['cToken']) for key, value in reserves_info.items()]
        key = f"cTokenUnderlyingPriceAll_{self.compound_info.get('lensAddress')}_{block_number}".lower()
        return {
    ...
    def get_all_markets(
        ...
        key = f"getAllMarkets_{self.compound_info.get('comptrollerAddress')}_{block_number}".lower()
        return {
