...
from defi_services.services.lending.lending_info.arbitrum.dforce_arbitrum import DFORCE_ARB
from defi_services.services.protocol_services import ProtocolServices

logger = logging.getLogger("Dforce Lending Pool State Service")


class CompoundInfo:
    mapping = {
        Chain.arbitrum: DFORCE_ARB
    }
...
class CompoundStateService(ProtocolServices):
    def __init__(self, state_service: StateQuerier, chain_id: str = "0xa4b1"):
        super().__init__()
        self.name = f"{chain_id}_{Lending.dforce}"
