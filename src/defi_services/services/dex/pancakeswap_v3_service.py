...
class PancakeSwapV3Service(DexProtocolServices):
    def __init__(self, state_service: StateQuerier, chain_id: str = '0x38'):
        ...
        self.nft_token_address = self.pool_info['NFT_token']
    ...
    def get_all_supported_lp_token(self):
        ...
        rpc_calls = {}
        for pid in range(1, int(pool_length / 10) + 1):
    ...
    def decode_lp_token_info(self, lp_token_info, response_data, block_number: int = "latest"):
        ...
        for lp_token, value in lp_token_info.items():
            ...
            total_liquidity = response_data.get(f'liquidity_{lp_token}_{block_number}_{self.chain_id}'.lower(),
                                                0)
            sqrt_priceX96 = response_data.get(f"slot0_{lp_token}_{block_number}_{self.chain_id}".lower())[0]
            price = self.convert_Q64_96_to_integer(sqrt_priceX96)
            cake_per_second = \
            response_data.get(f"getLatestPeriodInfoByPid_{lp_token}_{block_number}_{self.chain_id}".lower())[0]
            result[lp_token] = {
                "pid": pid,
                "token0": token0,
                'token1': token1,
    ...
    def get_all_nft_token_of_user_function(
        ...
        nft_contract = web3.eth.contract(abi=NFT_TOKEN_ABI, address=self.nft_token_address)
    ...
    def get_user_info_function(self, user: str, user_data: dict, stake: bool = True, block_number: int = "latest"):
        rpc_calls = {}
    ...
    def decode_user_info_function(self, user: str, user_data: dict, decoded_data: dict, token_price: dict = None,
            stake: bool = True,
    ...
    def convert_Q64_96_to_integer(self, sqrtPriceX96):
        integer_part = sqrtPriceX96 >> 96
        fractional_part = (sqrtPriceX96 & ((1 << 96) - 1)) / (2 ** 96)
