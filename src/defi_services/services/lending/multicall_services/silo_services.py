...
class SiloStateService(ProtocolServices):
    ...
    def get_dapp_asset_info(
        ...
        repository_contract = self._w3.eth.contract(
            address=self._w3.to_checksum_address(self.pool_info.get("repositoryAddress")), abi=self.repository_abi)
        result = {}
        if not tokens:
            tokens = self.pool_info.get("reservesList").keys()
        for token in tokens:
            silo_token = repository_contract.functions.getSilo(self._w3.to_checksum_address(token)).call(block_identifier=block_number)
            if not silo_token:
                continue
            contract = self._w3.eth.contract(address=self._w3.to_checksum_address(silo_token), abi=self.silo_abi)
    ...
    def get_apy_lending_pool_function_info(
        ...
        for token_address, reserve_info in reserves_info.items():
            pool_address = self._w3.to_checksum_address(reserve_info.get("pool"))
            multicall_calls.append(self.get_pool_function_info(
                pool=pool_address,
            ...
            for asset_address in reserve_info['assets']:
                params = [pool_address, self._w3.to_checksum_address(asset_address)]

                multicall_calls.append(W3Multicall.Call(
                    address=asset_address, abi=ERC20_ABI, fn_name="decimals", block_number=block_number))
                multicall_calls.append(self.get_lens_function_info(
                    fn_name='borrowAPY',
                    fn_paras=params,
                    block_number=block_number
                ))
    ...
    def get_rewards_balance_function_info(
        ...
        reward_token = self.pool_info.get('rewardToken')
        multicall_calls.append(W3Multicall.Call(
            address=reward_address, abi=self.reward_abi, fn_name="getUserUnclaimedRewards",
    ...
    def calculate_rewards_balance(self, wallet: str, reserves_info: dict, decoded_data: dict, block_number: int = "latest"):
        reward_address = self.pool_info.get('rewardAddress')
        reward_token = self.pool_info.get("rewardToken")
        get_reward_id = f"getUserUnclaimedRewards_{reward_address}_{wallet}_{block_number}".lower()
        decimals_id = f"decimals_{reward_token}_{block_number}".lower()
    ...
    def get_wallet_deposit_borrow_balance_function_info(
        ...
        for token, value in reserves_info.items():
            silo_pool = self._w3.to_checksum_address(value.get('pool'))
            ...
            for asset in assets:
                multicall_calls.append(W3Multicall.Call(
                    address=asset, abi=ERC20_ABI, fn_name="decimals", block_number=block_number))
                multicall_calls.append(self.get_lens_function_info(
                    "collateralBalanceOfUnderlying", [silo_pool, self._w3.to_checksum_address(asset), wallet], block_number))
                multicall_calls.append(self.get_lens_function_info(
                    "debtBalanceOfUnderlying", [silo_pool, self._w3.to_checksum_address(asset), wallet], block_number
                ))
    ...
    def calculate_wallet_deposit_borrow_balance(
        ...
        for token, value in reserves_info.items():
            ...
            if health_factor:
                key = f'getUserLiquidationThreshold_{self.pool_info["lensAddress"]}_{[silo_pool, wallet]}_{block_number}'.lower()
                liquidation_threshold = decoded_data.get(key) / 10 ** 18
            for asset in assets:
                deposit_key = f"collateralBalanceOfUnderlying_{self.pool_info['lensAddress']}_{[silo_pool, asset, wallet]}_{block_number}".lower()
                borrow_key = f"debtBalanceOfUnderlying_{self.pool_info['lensAddress']}_{[silo_pool, asset, wallet]}_{block_number}".lower()
    ...
    def get_lens_function_info(self, fn_name: str, fn_paras: list = None, block_number: int = "latest"):
        return W3Multicall.Call(
            address=self.pool_info['lensAddress'], abi=self.lens_abi, fn_name=fn_name, fn_paras=fn_paras, block_number=block_number)

    def get_pool_function_info(self, pool: str, fn_name: str, fn_paras: list = None, block_number: int = "latest"):
        return W3Multicall.Call(
            address=pool, abi=self.silo_abi, fn_name=fn_name, fn_paras=fn_paras, block_number=block_number)
