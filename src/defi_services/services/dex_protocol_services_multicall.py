...
class DexProtocolServices:
    ...
    def get_function_info(
        ...
        if Query.lp_token_list in query_types:
            ...
            multicall_calls.extend(self.get_all_supported_lp_token(limit_, supplied_data))

        if Query.farming_lp_token_list in query_types:
            limit_ = kwargs.get("number_lp")
            multicall_calls.extend(self.get_farming_supported_lp_token(limit_))
        if Query.important_lp_token_list in query_types:
            multicall_calls.extend(self.get_important_lp_token(supplied_data, block_number))

        if Query.lp_token_info in query_types:
            multicall_calls.extend(self.get_lp_token_function_info(supplied_data, block_number))

        if Query.token_pair_balance in query_types:
            multicall_calls.extend(self.get_balance_of_token_function_info(supplied_data, block_number))

        if Query.dex_user_nft in query_types and wallet and wallet != Token.native_token:
            multicall_calls.extend(self.get_all_nft_token_of_user_function(wallet, block_number))

        if Query.dex_user_info in query_types and wallet and wallet != Token.native_token:
            stake = kwargs.get("stake", False)
            multicall_calls.extend(self.get_user_info_function(wallet, supplied_data, stake, block_number))

        if Query.dex_user_token_balance in query_types:
            multicall_calls.extend(self.get_user_token_amount_function(wallet, supplied_data, block_number))

        if Query.protocol_reward in query_types and wallet and wallet != Token.native_token:
            multicall_calls.extend(self.get_rewards_balance_function_info(wallet, supplied_data, block_number))
    ...
    # Lp token function
    def get_lp_token_function_info(self, supplied_data, block_number: int = "latest") -> list:
        pass
    ...
    # Get lp list
    def get_all_supported_lp_token(self, limit: int = 10, supplied_data: dict = None) -> list:
        pass
    ...
    def get_farming_supported_lp_token(self, limit: int = 10) -> list:
        pass

    def decode_farming_supported_lp_token(self, decoded_data) -> dict:
        pass

    def get_important_lp_token(self, supplied_data, block_number) -> list:
        pass
    ...
    # Lp token liquidity
    def get_balance_of_token_function_info(self, supplied_data, block_number: int = "latest") -> list:
        pass
    ...
    def get_user_token_amount_function(self, wallet: str, supplied_data: dict, block_number: int = 'latest') -> list:
        pass
    ...
    def get_all_nft_token_of_user_function(
            self, wallet: str, block_number: int = "latest") -> list:
        pass
    ...
    # User reward
    def get_rewards_balance_function_info(self, wallet, supplied_data, block_number: int = "latest") -> list:
        pass
