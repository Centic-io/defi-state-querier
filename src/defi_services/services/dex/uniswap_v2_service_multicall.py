...
class UniswapV2Services(DexProtocolServices):
    def __init__(self, state_service: StateQuerier, chain_id: str = '0x1'):
        ...
        # multicall_address = Web3.to_checksum_address(MulticallContract.get_multicall_contract(self.chain_id))
        # self.w3_multicall = W3Multicall(web3=self._w3, address=multicall_address)
        if self.pool_info:
    ...
    def get_all_supported_lp_token(self, limit=10000, supplied_data: dict = None):
        ...
        factory_contract = _w3.eth.contract(
            address=_w3.to_checksum_address(factory_addr), abi=self.factory_abi)
        ...
        for pid in range(0, min(pool_length, limit)):
            multicall_calls.append(W3Multicall.Call(
                address=factory_addr, abi=self.factory_abi, fn_paras=pid, fn_name='allPairs', block_number="latest"))
    ...
    def get_important_lp_token(self, supplied_data, block_number: int = "latest"):
        ...
        for lp_token, value in lp_token_list.items():
            for fn_name in ["token0", "token1"]:
                multicall_calls.append(W3Multicall.Call(
                    address=lp_token, abi=LP_TOKEN_ABI, fn_name=fn_name, block_number=block_number))
        return multicall_calls
    ...
    def get_user_info_function(self, wallet: str, supplied_data, stake: bool = False, block_number: int = "latest"):
        ...
        for lp_token, value in lp_token_info.items():
            multicall_calls.append(W3Multicall.Call(
                address=lp_token, abi=LP_TOKEN_ABI, fn_name="balanceOf", fn_paras=self._w3.to_checksum_address(wallet),
                block_number=block_number))
            multicall_calls.append(W3Multicall.Call(
