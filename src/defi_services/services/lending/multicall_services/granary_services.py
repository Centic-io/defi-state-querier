...
class GranaryStateService(ProtocolServices):
    ...
    def get_rewards_balance_function_info(
        ...
        for token in reward_tokens:
            multicall_calls.append(self.get_function_rewarder_info(
                "getUserUnclaimedRewardsFromStorage",
                [self._w3.to_checksum_address(wallet), self._w3.to_checksum_address(token)], block_number))
            multicall_calls.append(W3Multicall.Call(
                address=token, abi=ERC20_ABI, fn_name="decimals", block_number=block_number
            ))
    ...
    def calculate_rewards_balance(
        ...
        for token in reward_tokens:
            ...
            key = f"getUserUnclaimedRewardsFromStorage_{self.pool_info['rewarder']}_{[wallet, token]}_{block_number}".lower()
    ...
    def get_function_rewarder_info(self, fn_name: str, fn_paras=None, block_number: int = 'latest'):
        return W3Multicall.Call(address=self.pool_info['rewarder'], abi=self.rewarder_abi, fn_name=fn_name,
                                fn_paras=fn_paras, block_number=block_number)
