...
class ValasStateService(ProtocolServices):
    ...
    def get_apy_lending_pool_function_info_deprecated(
        ...
        multicall_calls.append(self.get_function_incentive_info("rewardsPerSecond", block_number=block_number))
        multicall_calls.append(self.get_function_incentive_info("totalAllocPoint", block_number=block_number))
        for token_address, value in reserves_info.items():
            multicall_calls.append(self.get_function_lending_pool_info(
                "getReserveData", self._w3.to_checksum_address(token_address)))
            multicall_calls.append(self.get_function_incentive_info(
                "assets", self._w3.to_checksum_address(value['dToken']), block_number))
            multicall_calls.append(self.get_function_incentive_info(
                "assets", self._w3.to_checksum_address(value['tToken']), block_number))
            multicall_calls.append(W3Multicall.Call(
                address=value["tToken"], abi=ERC20_ABI, fn_name="totalSupply", block_number=block_number))
            multicall_calls.append(W3Multicall.Call(
                address=value["dToken"], abi=ERC20_ABI, fn_name="totalSupply", block_number=block_number))
    ...
    def calculate_apy_lending_pool_function_call_deprecated(
        ...
        total_supply_tokens = {}
        rewards_per_second_key = f'rewardsPerSecond_{self.pool_info["stakedIncentiveAddress"]}_{block_number}'.lower()
        total_alloc_point_key = f'totalAllocPoint_{self.pool_info["stakedIncentiveAddress"]}_{block_number}'.lower()
        ...
        for token_address in reserves_info:
            ...
            decimals_call_id = f"decimals_{token_address}_{block_number}".lower()
            atoken_assets_key = f'assets_{self.pool_info["stakedIncentiveAddress"]}_{atoken}_{block_number}'.lower()
            debt_token_assets_key = f'assets_{self.pool_info["stakedIncentiveAddress"]}_{debt_token}_{block_number}'.lower()
            atoken_total_supply_key = f'totalSupply_{atoken}_{block_number}'.lower()
            debt_token_total_supply_key = f'totalSupply_{debt_token}_{block_number}'.lower()
    ...
    def get_wallet_deposit_borrow_balance_function_info(
        ...
        for token in reserves_info:
            ...
            multicall_calls.append(W3Multicall.Call(
                address=value["sdToken"], abi=ERC20_ABI, fn_name="balanceOf", fn_paras=wallet, block_number=block_number
            ))
            multicall_calls.append(W3Multicall.Call(
    ...
    def get_rewards_balance_function_info(
        ...
        multicall_calls: List['W3Multicall.Call'] = []
        multicall_calls.append(self.get_function_multi_fee_distribution_info(
            "earnedBalances", self._w3.to_checksum_address(wallet), block_number))
    ...
    def calculate_rewards_balance(
        ...
        key = f'earnedBalances_{self.pool_info["multiFeeAddress"]}_{wallet}_{block_number}'.lower()
    ...
    def get_function_lending_pool_info(self, fn_name: str, fn_paras=None, block_number: int = 'latest'):
        return W3Multicall.Call(
            address=self.pool_info["address"], abi=self.lending_abi, fn_name=fn_name,
    ...
    def get_function_incentive_info(self, fn_name: str, fn_paras=None, block_number: int = 'latest'):
        return W3Multicall.Call(
            address=self.pool_info["stakedIncentiveAddress"], abi=self.incentive_abi, fn_name=fn_name,
            fn_paras=fn_paras, block_number=block_number)

    def get_function_oracle_info(self, fn_name: str, fn_paras=None, block_number: int = 'latest'):
        return W3Multicall.Call(
            address=self.pool_info["oracleAddress"], abi=self.oracle_abi, fn_name=fn_name,
            fn_paras=fn_paras, block_number=block_number)

    def get_function_multi_fee_distribution_info(self, fn_name: str, fn_paras=None, block_number: int = 'latest'):
        return W3Multicall.Call(
            address=self.pool_info["multiFeeAddress"], abi=self.multi_fee_distribution_abi, fn_name=fn_name,
            fn_paras=fn_paras, block_number=block_number)
