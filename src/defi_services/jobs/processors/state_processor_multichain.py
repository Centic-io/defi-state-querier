...
class StateProcessor:
    def __init__(self, provider_uri: str, chain_id: str):
        self.state_querier = StateQuerier(provider_uri, chain_id)
    ...
    def init_rpc_call_information(
        ...
        if self.check_address(entity_id):
            if Query.token_balance == query_type:
                multicall_calls.extend(self.token_service.get_function_info(
                    wallet, entity_id, block_number
                ))
                tokens.append(entity_id)

            if Query.nft_balance == query_type:
                multicall_calls.extend(self.nft_service.get_function_info(
                    wallet, entity_id, block_number
                ))

        if entity_id in self.services:
            entity_service = self.services.get(entity_id)
            multicall_calls.extend(entity_service.get_function_info([query_type], wallet, block_number, **kwargs))
            tokens += entity_service.get_token_list()
        queries[entity_id] = multicall_calls
    ...
    def run(self, address: str, queries: list, block_number: int = 'latest', batch_size: int = 100):
        ...
        for query in queries:
            ...
            multicall_calls, _ = self.init_rpc_call_information(
