...
from defi_services.abis.lending.tenderfi.tenderfi_comptroller_abi import   TENDERFI_COMPTROLLER_ABI
...
from defi_services.services.lending.lending_info.arbitrum.tenderfi_arbitrum import TENDERFI_ARB
from defi_services.services.protocol_services import ProtocolServices

logger = logging.getLogger("Tender Finance Lending Pool State Service")


class TenderFiInfo:
    mapping = {
        Chain.arbitrum: TENDERFI_ARB
    }


class TenderFiStateService(ProtocolServices):
    def __init__(self, state_service: StateQuerier, chain_id: str = "0xa4b1"):
        super().__init__()
        self.name = f"{chain_id}_{Lending.tenderfi}"
        self.chain_id = chain_id
        self.pool_info = TenderFiInfo.mapping.get(chain_id)
        self.state_service = state_service
        self.lens_abi = CREAM_LENS_ABI
        self.comptroller_abi = TENDERFI_COMPTROLLER_ABI
