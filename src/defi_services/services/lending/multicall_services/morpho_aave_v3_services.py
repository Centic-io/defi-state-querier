...
class MorphoAaveV3StateService(MorphoCompoundStateService):
    ...
    def get_wallet_deposit_borrow_balance_function_info(
        ...
        for token, value in reserves_info.items():
            ...
            multicall_calls.append(self.get_comptroller_function_info(
                "borrowBalance", [self._w3.to_checksum_address(token), wallet], block_number))
            multicall_calls.append(self.get_comptroller_function_info(
                "collateralBalance", [self._w3.to_checksum_address(token), wallet], block_number))
    ...
    def calculate_wallet_deposit_borrow_balance(
        ...
        for token, value in reserves_info.items():
            ...
            if token == Token.native_token:
                underlying = Token.wrapped_token.get(self.chain_id)
            get_total_deposit_id = f"collateralBalance_{self.pool_info['comptrollerAddress']}_{[token, wallet]}_{block_number}".lower()
            get_total_borrow_id = f"borrowBalance_{self.pool_info['comptrollerAddress']}_{[token, wallet]}_{block_number}".lower()
