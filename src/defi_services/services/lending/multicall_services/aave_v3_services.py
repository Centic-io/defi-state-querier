...
class AaveV3StateService(AaveV2StateService):
    ...
    def get_apy_lending_pool_function_info_deprecated(
        ...
        for token_address, value in reserves_info.items():
            for reward_token in reward_tokens:
                reward_token = self._w3.to_checksum_address(reward_token)
                multicall_calls.append(self.get_function_incentive_info(
                    "getRewardsData", [self._w3.to_checksum_address(value['tToken']), reward_token], block_number
                ))
                multicall_calls.append(self.get_function_incentive_info(
                    "getRewardsData", [self._w3.to_checksum_address(value['dToken']), reward_token], block_number
                ))
                multicall_calls.append(self.get_function_incentive_info(
                    "getRewardsData", [self._w3.to_checksum_address(value['sdToken']), reward_token], block_number
                ))

            multicall_calls.append(self.get_function_lending_pool_info(
                "getReserveData", [self._w3.to_checksum_address(token_address)], block_number
            ))
            ...
            multicall_calls.append(W3Multicall.Call(
                address=value["sdToken"], abi=ERC20_ABI, fn_name="totalSupply", block_number=block_number))
            multicall_calls.append(W3Multicall.Call(
    ...
    def get_rewards_balance_function_info(
        ...
        call = self.get_function_incentive_info("getAllUserRewards", [tokens, Web3.to_checksum_address(wallet)], block_number)
        call.id = f"getAllUserRewards_{self.pool_info['stakedIncentiveAddress']}_{wallet}_{block_number}".lower()
    ...
    def calculate_rewards_balance(
        ...
        key = f"getAllUserRewards_{self.pool_info['stakedIncentiveAddress']}_{wallet}_{block_number}".lower()
