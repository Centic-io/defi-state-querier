...
class FluxStateService(CompoundStateService):
    ...
    def get_wallet_deposit_borrow_balance_function_info(
        ...
        for token, value in reserves_info.items():
            ...
            multicall_calls.append(self.get_ctoken_function_info(
                ctoken=ctoken, fn_name="borrowBalanceCurrent", fn_paras=wallet, block_number=block_number
            ))
            multicall_calls.append(self.get_ctoken_function_info(
                ctoken=ctoken, fn_name="balanceOfUnderlying", fn_paras=wallet, block_number=block_number
            ))
    ...
    def get_token_deposit_borrow_balance_function_info(
        ...
        for token, value in reserves_info.items():
            ...
            multicall_calls.append(W3Multicall.Call(
                address=ctoken, abi=ERC20_ABI, fn_name="exchangeRateCurrent", block_number=block_number))
