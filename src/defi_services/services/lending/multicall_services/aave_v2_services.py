...
class AaveV2StateService(ProtocolServices):
    ...
    def get_apy_lending_pool_function_info(
        ...
        for token_address, value in reserves_info.items():
            multicall_calls.append(W3Multicall.Call(
                address=self.pool_info['address'], abi=self.lending_abi, fn_name="getReserveData",
                fn_paras=self._w3.to_checksum_address(token_address), block_number=block_number
            ))
            multicall_calls.append(W3Multicall.Call(
    ...
    def get_wallet_deposit_borrow_balance_function_info(
        ...
        for token in reserves_info:
            ...
            multicall_calls.append(W3Multicall.Call(
                address=value["tToken"], abi=ERC20_ABI, fn_name="balanceOf",
                fn_paras=self._w3.to_checksum_address(wallet), block_number=block_number
            ))
            multicall_calls.append(W3Multicall.Call(
                address=value["dToken"], abi=ERC20_ABI, fn_name="balanceOf",
                fn_paras=self._w3.to_checksum_address(wallet), block_number=block_number
            ))
            multicall_calls.append(W3Multicall.Call(
                address=value["sdToken"], abi=ERC20_ABI, fn_name="balanceOf",
                fn_paras=self._w3.to_checksum_address(wallet), block_number=block_number
            ))
            multicall_calls.append(W3Multicall.Call(
    ...
    def calculate_health_factor(
        ...
        key = f'getUserAccountData_{self.pool_info["address"]}_{wallet}_{block_number}'.lower()
