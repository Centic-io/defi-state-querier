...
class MorphoCompoundStateService(ProtocolServices):
    ...
    def get_apy_lending_pool_function_info(
        ...
        for token_address, value in reserves_info.items():
            ...
            c_token = self._w3.to_checksum_address(value['cToken'])
    ...
    def get_reserve_tokens_metadata(
        ...
        for token_address, reserve_info in reserves_info.items():
            ...
            market_data_query_id = f"getMainMarketData_{self.pool_info['lensAddress']}_{ctoken}_{block_number}".lower()
    ...
    def get_rewards_balance_function_info(
        ...
        multicall_call = self.get_lens_function_info("getUserUnclaimedRewards", params, block_number)
        multicall_call.id = f"getUserUnclaimedRewards_{self.pool_info['lensAddress']}_{wallet}_{block_number}".lower()
        return [multicall_call]

    def calculate_rewards_balance(
            self, wallet: str, reserves_info: dict, decoded_data: dict, block_number: int = "latest"):
        get_reward_id = f"getUserUnclaimedRewards_{self.pool_info['lensAddress']}_{wallet}_{block_number}".lower()
    ...
    def get_wallet_deposit_borrow_balance_function_info(
        ...
        for token, value in reserves_info.items():
            underlying = token
            ctoken = self._w3.to_checksum_address(value.get(self.market_key))
    ...
    def calculate_wallet_deposit_borrow_balance(
        ...
        for token, value in reserves_info.items():
            ...
            if token == Token.native_token:
                underlying = Token.wrapped_token.get(self.chain_id)
            get_total_deposit_id = f"getCurrentSupplyBalanceInOf_{self.pool_info['lensAddress']}_{[ctoken, wallet]}_{block_number}".lower()
            get_total_borrow_id = f"getCurrentBorrowBalanceInOf_{self.pool_info['lensAddress']}_{[ctoken, wallet]}_{block_number}".lower()
    ...
    def get_lens_function_info(self, fn_name: str, fn_paras: list, block_number: int = "latest"):
        return W3Multicall.Call(
            address=self.pool_info['lensAddress'], abi=self.lens_abi, fn_name=fn_name,
    ...
    def get_comptroller_function_info(self, fn_name: str, fn_paras: list, block_number: int = "latest"):
        return W3Multicall.Call(
            address=self.pool_info['comptrollerAddress'], abi=self.comptroller_abi, fn_name=fn_name,
            fn_paras=fn_paras, block_number=block_number)

    def get_ctoken_function_info(self, ctoken: str, fn_name: str, fn_paras: list = None, block_number: int = "latest"):
        return W3Multicall.Call(
            address=ctoken, abi=CTOKEN_ABI, fn_name=fn_name, fn_paras=fn_paras, block_number=block_number)
