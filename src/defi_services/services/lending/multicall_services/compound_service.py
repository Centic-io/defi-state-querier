...
class CompoundStateService(ProtocolServices):
    ...
    def get_dapp_asset_info(
        ...
        comptroller_contract = self._w3.eth.contract(
            address=self._w3.to_checksum_address(self.pool_info.get("comptrollerAddress")), abi=self.comptroller_abi)
        ctokens = []
    ...
    def get_reserve_tokens_metadata_deprecated(
        ...
        for token, reserve_info in reserves_info.items():
            ctoken = reserve_info.get('cToken')
            metadata_id = f'cTokenMetadata_{self.pool_info["lensAddress"]}_{ctoken}_{block_number}'.lower()
    ...
    def get_rewards_balance_function_info(
        ...
        comptroller = self._w3.to_checksum_address(self.pool_info.get("comptrollerAddress"))
        multicall_call = self.get_lens_function_info(
            "getCompBalanceMetadataExt", [reward_token, comptroller, self._w3.to_checksum_address(wallet)], block_number)
        multicall_call.id = f'getCompBalanceMetadataExt_{self.pool_info["lensAddress"]}_{wallet}_{block_number}'.lower()
    ...
    def calculate_rewards_balance(self, wallet: str, reserves_info: dict, decoded_data: dict, block_number: int = "latest"):
        get_reward_id = f'getCompBalanceMetadataExt_{self.pool_info["lensAddress"]}_{wallet}_{block_number}'.lower()
    ...
    @staticmethod
    def get_ctoken_function_info(ctoken: str, fn_name: str, fn_paras: list = None, block_number: int = "latest"):
        return W3Multicall.Call(
