...
class VenusStateService(CompoundStateService):
    ...
    def calculate_rewards_balance(
        ...
        # get_reward_id = f"pendingRewards_{self.name}_{wallet}_{block_number}".lower()
        return_data = contract.functions.pendingRewards(w3.to_checksum_address(wallet), w3.to_checksum_address(
            self.pool_info.get("comptrollerAddress"))).call(block_identifier=block_number)
