...
class UniswapV3Services(DexProtocolServices):
    ...
    def decode_balance_of_token_function_info(
        ...
        for lp_token, value in supplied_data['lp_token_info'].items():
            ...
            amount0, amount1 = self.get_token_amount_of_user(liquidity=liquidity, sqrt_price_x96=sqrt_price_x96, tick=tick,
        ...
        for token_id, value in user_data.items():
            ...
            if liquidity > 0:
                ...
                if price and tick:
                    ...
                    token0_amount, token1_amount = self.get_token_amount_of_user(liquidity=liquidity,
    ...
    def _get_amount0_delta1(self, sqrt_ratio_ax96, sqrt_ratio_bx96, liquidity, round_up):
        ...
        if sqrt_ratio_ax96 < 0:
            ...
            if round_up:
                x = self._mul_div_rounding_up(numerator1, numerator2, sqrt_ratio_bx96)
                return self._div_rounding_up(x, sqrt_ratio_ax96)
            else:
                return (numerator1 * numerator2 / sqrt_ratio_bx96) / sqrt_ratio_ax96

    def _get_amount1_delta1(self, sqrt_ratio_ax96, sqrt_ratio_bx96, liquidity, round_up):
        if sqrt_ratio_ax96 > sqrt_ratio_bx96:
            sqrt_ratio_ax96, sqrt_ratio_bx96 = sqrt_ratio_bx96, sqrt_ratio_ax96
        if round_up:
            x = self._mul_div_rounding_up(liquidity, sqrt_ratio_bx96 - sqrt_ratio_ax96, FixedPoint96.Q96)
            return x
        else:
            return liquidity * (sqrt_ratio_bx96 - sqrt_ratio_ax96) / FixedPoint96.Q96

    def _get_amount0_delta(self, sqrt_ratio_ax96, sqrt_ratio_bx96, liquidity):
        if liquidity < 0:
            return -self._get_amount0_delta1(sqrt_ratio_ax96, sqrt_ratio_bx96, -liquidity, False)
        else:
            return self._get_amount0_delta1(sqrt_ratio_ax96, sqrt_ratio_bx96, liquidity, True)

    def _get_amount1_delta(self, sqrt_ratio_ax96, sqrt_ratio_bx96, liquidity):
        if liquidity < 0:
            return -self._get_amount1_delta1(sqrt_ratio_ax96, sqrt_ratio_bx96, -liquidity, False)
        else:
            return self._get_amount1_delta1(sqrt_ratio_ax96, sqrt_ratio_bx96, liquidity, True)

    def get_token_amount_of_user(self, liquidity, sqrt_price_x96, tick, tick_lower, tick_upper):

        amount0, amount1 = 0, 0
        if liquidity != 0:
            if tick < tick_lower:
                amount0 = self._get_amount0_delta(self._get_sqrt_ratio_at_tick(tick_lower),
                                                  self._get_sqrt_ratio_at_tick(tick_upper), liquidity)
            elif tick < tick_upper:
                amount0 = self._get_amount0_delta(sqrt_price_x96, self._get_sqrt_ratio_at_tick(tick_upper), liquidity)
                amount1 = self._get_amount1_delta(self._get_sqrt_ratio_at_tick(tick_lower), sqrt_price_x96, liquidity)
            else:
                amount1 = self._get_amount1_delta(self._get_sqrt_ratio_at_tick(tick_lower),
                                                  self._get_sqrt_ratio_at_tick(tick_upper),
                                                  liquidity)
        return amount0, amount1

    def get_token_amount_of_pool(self, liquidity, tick_lower, tick_upper):
