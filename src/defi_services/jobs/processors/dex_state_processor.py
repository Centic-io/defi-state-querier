...
class DexStateProcessor:
    def __init__(self, mongo_klg=None, chain_id="0x38", provider_uri="https://bsc-dataseed1.binance.org/"):
    ...
    def run(self, queries: list, wallet, batch_size: int = 100, max_workers: int = 8, ignore_error: bool = False,
        ...
        for query in queries:
            ...
            if entity_id in self.services:
                ...
                if query_type == 'userinfo' and len(lp_tokens) != 0:
                    ...
                    user_info = self.run_user_info_with_specific_lp(wallet, lp_tokens, data, batch_size, max_workers,
                                                                    ignore_error)

                    with open(f'{entity_id}_{version}_{self.chain_id}_user_info.json', "w") as f:
                        json.dump(user_info, f)

    def run_lp_token(self, lp_token_list, list_farms_info, batch_size: int = 100, max_workers: int = 8,
    ...
    def run_user_info_with_specific_lp(
            self, wallet, lp_tokens, data, batch_size: int = 100, max_workers: int = 8, ignore_error: bool = False):
        ...
        user_info = self.state_querier.query_state_data(
