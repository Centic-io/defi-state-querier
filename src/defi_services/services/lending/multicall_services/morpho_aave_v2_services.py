...
class MorphoAaveV2StateService(MorphoCompoundStateService):
    ...
    def get_apy_lending_pool_function_info(
        ...
        for token_address, value in reserves_info.items():
            ...
            multicall_calls.append(self.get_lens_function_info(
                'getMainMarketData', self._w3.to_checksum_address(a_token), block_number=block_number))
    ...
    def get_reserve_tokens_metadata(
        ...
        for token_address, reserve_info in reserves_info.items():
            get_reserve_data_call_id = f'getReserveData_{self.aave_info["address"]}_{token_address}_{block_number}'.lower()
            reserve_data = decoded_data.get(get_reserve_data_call_id)

            a_token = reserve_data[7].lower()
            market_data_query_id = f"getMainMarketData_{self.pool_info['lensAddress']}_{a_token}_{block_number}".lower()
    ...
    def get_function_lending_pool_info(self, fn_name: str, fn_paras=None, block_number: int = 'latest'):
        return W3Multicall.Call(
            address=self.aave_info['address'], abi=self.lending_abi, fn_name=fn_name,
